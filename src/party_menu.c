...
static void UpdatePartySelectionSingleLayout(s8 *slotPtr, s8 movementDir)
{
    ...
    u8 partyCount = gPartiesCount[partyTrainer];
    ...
    if (gPartyMenu.layout != PARTY_LAYOUT_SINGLE)
    {
        ...
        case MENU_DIR_UP:
            if (*slotPtr == 0)
                ...
                *slotPtr = sPartyMenuInternal->chooseHalf ? PARTY_SIZE : partyCount - 1;
            else
                (*slotPtr)--;
            break;
        case MENU_DIR_DOWN:
            if (*slotPtr == PARTY_SIZE + 1)
                *slotPtr = 0;
            else if (*slotPtr == partyCount - 1)
                *slotPtr = sPartyMenuInternal->chooseHalf ? PARTY_SIZE : PARTY_SIZE + 1;
            else
                (*slotPtr)++;
            break;
        case MENU_DIR_RIGHT:
    ...
    {
        // Custom two-column layout. PARTY_SIZE + 1 is Cancel, PARTY_SIZE is Confirm.
        switch (movementDir)
        {
        case MENU_DIR_UP:
            if (*slotPtr == 0)
        ...
        case MENU_DIR_DOWN:
            ...
            break;
        case MENU_DIR_RIGHT:
            if (partyCount != 1 && *slotPtr < partyCount && *slotPtr % 2 == 0 && *slotPtr + 1 < partyCount)
                (*slotPtr)++;
            break;
        case MENU_DIR_LEFT:
            if (*slotPtr < partyCount && *slotPtr % 2 == 1)
                (*slotPtr)--;
            break;
        }
    }
...
void DisplayPartyMenuStdMessage(u32 stringId)
{
    ...
    if (stringId != PARTY_MSG_NONE)
    {
        ...
        if (GET_LANGUAGE() == PT)
            StringExpandPlaceholders(gStringVar4, sActionStringTablePt[stringId]);
        else if (GET_LANGUAGE() == ES)
            StringExpandPlaceholders(gStringVar4, sActionStringTableSpa[stringId]);
        else
...
{
    if (item == ITEM_PROTEIN || item == ITEM_IRON || item == ITEM_CARBOS || item == ITEM_CALCIUM || item == ITEM_HP_UP || item == ITEM_ZINC || item == ITEM_HEALTH_WING || item == ITEM_MUSCLE_WING || item == ITEM_RESIST_WING || item == ITEM_GENIUS_WING || item == ITEM_CLEVER_WING || item == ITEM_SWIFT_WING)
    {
        return TRUE;
    }
    return FALSE;
...
void Task_AbilityCapsule(u8 taskId)
{
    ...
    case 0:
        // Can't use.
        if (GetSpeciesAbility(tSpecies, 0) == GetSpeciesAbility(tSpecies, 1) || GetSpeciesAbility(tSpecies, 1) == 0 || tAbilityNum > 1 || !tSpecies)
        {
            gPartyMenuUseExitCallback = FALSE;
            PlaySE(SE_SELECT);
...
void ItemUseCB_EvolutionStone(u8 taskId, TaskFunc task)
{
    PlaySE(SE_SELECT);
    gCB2_AfterEvolution = gPartyMenu.exitCallback;
    if (ExecuteTableBasedItemEffect(&gParties[B_TRAINER_PLAYER][gPartyMenu.slotId], gSpecialVar_ItemId, gPartyMenu.slotId, 0, 0, 1))
    {
        gPartyMenuUseExitCallback = FALSE;
        DisplayPartyMenuMessage(gText_WontHaveEffect, TRUE);
        ScheduleBgCopyTilemapToVram(2);
...
void CursorCb_MoveItemCallback(u8 taskId)
{
    ...
    case 1: // User hit A on a Pokemon
        ...
        if (GetMonData(&gParties[B_TRAINER_PLAYER][gPartyMenu.slotId2], MON_DATA_IS_EGG) || gPartyMenu.slotId == gPartyMenu.slotId2)
        {
            PlaySE(SE_FAILURE);
            return;
        }
